...
def internet_element_taggin(text, preserve_con="OnlyInHashtags"):
        """
        ...
        elif preserve_con == "OnlyInHashtags":
                # Its necesary to detect RT first
...
def taggin_prices(text, preserve_con=False):
        """
        Detect and tagg numbers.
        ...
        """
        if preserve_con:
                tagged_text = re.sub(PR,r" <PRICE=\1> ", text)
        else:
                tagged_text = re.sub(PR,r" <PRICE> ", text)
        return tagged_text
